...
export const AgentSelector: Component = () => {
  ...
  return (
    <div class="relative" ref={dropdownRef}>
      <button
        type="button"
        class="flex items-center gap-2 px-3 py-1.5 bg-surface-2 border border-surface-3 rounded-md text-xs text-foreground cursor-pointer hover:bg-surface-3 transition-colors"
        ...
        <div class="absolute bottom-full left-0 mb-1 w-64 bg-surface-0 border border-surface-3 rounded-lg shadow-lg z-50 overflow-hidden">
          <For each={acpStore.availableAgents}>
            {(agent) => (
              <button
                type="button"
                class={`w-full text-left px-3 py-2.5 border-b border-surface-2 last:border-b-0 transition-colors ${
                  agent.available
              ...
              >
                <div class="flex items-center justify-between mb-1">
                  ...
                  <Show when={!agent.available}>
                    <span class="text-[10px] px-1.5 py-0.5 bg-surface-3 text-muted-foreground rounded">
                      Coming Soon
                    </span>
                  </Show>
                  <Show
                ...
                <Show when={!agent.available && agent.unavailableReason}>
                  <p class="text-[10px] text-destructive m-0 mt-1">
