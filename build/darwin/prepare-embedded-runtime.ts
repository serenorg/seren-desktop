// ABOUTME: Downloads and prepares Node.js and Git binaries for macOS embedded runtime.
// ABOUTME: Called during the build process to bundle these tools with the Tauri app bundle.
...
async function prepareGit(options: DownloadOptions): Promise<string> {
	...
	// On macOS, we create a wrapper that uses system git
	// Building git from source requires additional dependencies (Xcode CLT)
	// Most macOS systems have git available via Xcode or Homebrew
...
if command -v /usr/bin/git &> /dev/null; then
	exec /usr/bin/git "$@"
elif command -v /opt/homebrew/bin/git &> /dev/null; then
	exec /opt/homebrew/bin/git "$@"
elif command -v /usr/local/bin/git &> /dev/null; then
	exec /usr/local/bin/git "$@"
else
	echo "Error: Git not found. Please install git via Xcode Command Line Tools or Homebrew." >&2
	exit 1
fi
`;
...
export async function prepareEmbeddedRuntime(arch: 'x64' | 'arm64', outputDir: string): Promise<{ nodeDir: string; gitDir: string }> {
	console.log(`Preparing embedded runtime for macOS ${arch}...`);
...
if (require.main === module) {
	...
	const outputDir = process.argv[3] || path.join(process.cwd(), '.build', 'embedded-runtime', `darwin-${arch}`);
